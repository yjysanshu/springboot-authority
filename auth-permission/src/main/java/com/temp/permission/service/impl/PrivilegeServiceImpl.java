...
public class PrivilegeServiceImpl {
    ...
    public Map getRoleMenu(Integer roleId) throws IOException {
        ...
        List<MenuVO> list= this.getMenuByRoleList(Collections.singletonList(role.getRoleParentId()));
     ...
     * @throws IOException -
     */
    public List<MenuVO> getMenuByRoleList(List<Integer> roleIds) throws IOException {
        List<MenuVO> list = menuService.getMenuByParentId(BackendConst.PARENT_ID_DEFAULT);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            List<MenuVO> listChildren = list.get(i).getChildren();
            List<MenuVO> removeChildrenList = new ArrayList<>();
            ...
            for (MenuVO menu : removeChildrenList) {
                listChildren.remove(menu);
            }
        }
        for (MenuVO menu : removeList) {
            list.remove(menu);
        }
     ...
     * @return array
     */
    private List<MenuVO> checkMenuPrivilege(List<MenuVO> menuList, List<Integer> roleIds) {
