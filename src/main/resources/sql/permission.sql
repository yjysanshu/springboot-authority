...
INSERT INTO `back_user` (`user_phone`, `user_name`, `user_email`, `user_password`)
...
INSERT INTO `back_role` VALUES (1,0,'超级管理员','拥有所有权限',0,1,'2018-06-10 14:31:45','2018-06-10 14:31:45','','');
...
INSERT INTO `back_role_user` VALUES (1,1,1,'2018-06-19 17:07:09','2018-06-19 17:07:09','','');

CREATE TABLE `back_resource` (
...
INSERT INTO `back_resource`(resource_id, resource_parent_id, resource_data)
VALUES
...
BEGIN
	...
	DECLARE roleUser CURSOR FOR SELECT role_user_role_id FROM back_role_user WHERE role_user_user_id=userId;
  	...
  	if roleIds_del != '' then
  	  SET @del_sql = CONCAT("delete from back_role_user where role_user_role_id in (", roleIds_del, ") and role_user_user_id=", userId);
  	...
  	if roleIds_add != '' then
    	SET num = (length(roleIds_add) - length(replace(roleIds_add,',',''))) + 1;
  		...
  		WHILE i <= num DO
  			...
  			insert into back_role_user(role_user_role_id, role_user_user_id) values(roleId_add, userId);
  			SET i = i + 1;
  		END WHILE;
  	end if;
END;;
...
BEGIN
	...
	DECLARE userRole CURSOR FOR SELECT role_user_user_id FROM back_role_user WHERE role_user_role_id=roleId;
  	DECLARE CONTINUE HANDLER FOR NOT FOUND SET s=1;
  	...
  	if userIds_del != '' then
  		SET @del_sql = CONCAT("delete from back_role_user where role_user_user_id in (", userIds_del, ") and role_user_role_id=", roleId);
  		prepare mainStmt from @del_sql;
  		execute mainStmt;
  		deallocate prepare mainStmt;
  	end if;
  	...
  	if userIds_add != '' then
    	SET num = (length(userIds_add) - length(replace(userIds_add,',',''))) + 1;
  		...
  		WHILE i <= num DO
  			...
  			insert into back_role_user(role_user_role_id, role_user_user_id) values(roleId, userId_add);
  			SET i = i + 1;
  		END WHILE;
  	end if;
END;;
